...
describe('parseBloxAiEvent — happy path for each variant', () => {
  test.each([
    ...
    expect(parseBloxAiEvent(frame).type).toBe((frame as any).type);
  });
});
...
describe('parseBloxAiEvent — tier and severity guards', () => {
  test('verdict.severity yellow is accepted', () => {
    expect(parseBloxAiEvent({ type: 'verdict', payload: { summary: 'meh', severity: 'yellow' } }).type).toBe('verdict');
  });
  test('recommended_action tier 3 is accepted', () => {
    expect(
      parseBloxAiEvent({ type: 'recommended_action', action_id: 'a', action_name: 'reset', args: {}, reasoning: 'x', confidence: 0.9, tier: 3, approval_token: 'k'.repeat(80) }).type,
