...
/**
 * Ported from apps/box/src/utils/__tests__/bleAiClient.test.ts — BleAiClient over any BleCommandWriter.
 */
import { describe, expect, test, vi } from 'vitest';
import { BleAiClient } from '../bleAiClient';
import { BleStreamTimeoutError, type BleCommandWriter } from '../ble';

function makeBleManager(behavior: { onWrite?: (cmd: string, onStreamFrame?: (frame: any) => void) => Promise<any> } = {}) {
  const onWrite = behavior.onWrite ?? (() => Promise.resolve({}));
  const writeToBLEAndWaitForResponse = vi.fn(
    (cmd: string, _peripheral?: string, _svc?: string, _char?: string, _timeout?: number, onStreamFrame?: (f: any) => void) => onWrite(cmd, onStreamFrame),
  );
  return { writeToBLEAndWaitForResponse } as unknown as BleCommandWriter & { writeToBLEAndWaitForResponse: ReturnType<typeof vi.fn> };
}
...
describe('BleAiClient.health()', () => {
  test('ok=true when ai/status command resolves', async () => {
    const c = new BleAiClient(makeBleManager({ onWrite: () => Promise.resolve({}) }), 'periph');
  ...
  test('ok=false when ai/status command rejects', async () => {
    const c = new BleAiClient(makeBleManager({ onWrite: () => Promise.reject(new Error('no peer')) }), 'periph');
    const r = await c.health();
...
describe('BleAiClient.runAi — stream frame routing + lifecycle', () => {
  ...
  test('updates sessionId from session_started event', () => {
    let captured: ((frame: any) => void) | undefined;
    const mgr = makeBleManager({
      onWrite: (_cmd, onStreamFrame) => {
        captured = onStreamFrame;
    ...
    const c = new BleAiClient(mgr, 'periph');
    const session = c.runAi('hi', undefined, { onEvent: vi.fn(), onComplete: vi.fn(), onError: vi.fn() });
  ...
  test('"Another command is in progress" → ble-busy error (not transient)', async () => {
    const mgr = makeBleManager({ onWrite: () => Promise.reject(new Error('Another command is in progress')) });
    const c = new BleAiClient(mgr, 'periph');
    const onError = vi.fn();
    const onComplete = vi.fn();
  ...
  test('BleStreamTimeoutError → network error (transient)', async () => {
    const mgr = makeBleManager({ onWrite: () => Promise.reject(new BleStreamTimeoutError('timeout', [])) });
    const c = new BleAiClient(mgr, 'periph');
    const onError = vi.fn();
    c.runAi('hi', undefined, { onEvent: vi.fn(), onError });
    await new Promise((res) => setTimeout(res, 5));
  ...
  test('cancel() prevents onError + onComplete from firing afterwards', async () => {
    let resolveWrite: (v?: any) => void = () => undefined;
    const mgr = makeBleManager({ onWrite: () => new Promise((res) => (resolveWrite = res)) });
    const c = new BleAiClient(mgr, 'periph');
    const onError = vi.fn();
    const onComplete = vi.fn();
...
describe('BleAiClient.userReply / phoneContext', () => {
  ...
  test('userReply rejects with ble-busy when channel is in use', async () => {
    const c = new BleAiClient(makeBleManager({ onWrite: () => Promise.reject(new Error('Another command is in progress')) }), 'periph');
    await expect(c.userReply('s', 'q', 'r')).rejects.toMatchObject({ kind: 'http-busy', transient: false });
  });

  test('phoneContext rejects with network on generic BLE failure', async () => {
    const c = new BleAiClient(makeBleManager({ onWrite: () => Promise.reject(new Error('connection lost')) }), 'periph');
    await expect(c.phoneContext('s', { foo: 1 })).rejects.toMatchObject({ kind: 'network', transient: true });
  });
});

describe('BleAiClient.executeAction', () => {
  test('successful execution_result payload', async () => {
    const mgr = makeBleManager({ onWrite: () => Promise.resolve({ type: 'execution_result', action_id: 'a1', success: true, duration_ms: 200 }) });
    const c = new BleAiClient(mgr, 'periph');
  ...
  test('handles JSON-string response from BLE assembler', async () => {
    const mgr = makeBleManager({
      onWrite: () => Promise.resolve(JSON.stringify({ type: 'execution_result', action_id: 'a1', success: true, duration_ms: 100 })),
    });
    const c = new BleAiClient(mgr, 'periph');
    const r = await c.executeAction({ action_id: 'a1', approval_token: 'tok' });
    expect(r.ok).toBe(true);
  ...
  test('malformed response → sse-malformed', async () => {
    const c = new BleAiClient(makeBleManager({ onWrite: () => Promise.resolve('not-execution-result') }), 'periph');
    const r = await c.executeAction({ action_id: 'a1', approval_token: 'tok' });
    expect(r.ok).toBe(false);
  ...
  test('includes security_code only when provided', async () => {
    ...
    expect(JSON.parse(writes[0]!).args.security_code).toBeUndefined();
    expect(JSON.parse(writes[1]!).args.security_code).toBe('1234');
  });
});

describe('BleAiClient.cancel — best-effort swallows errors', () => {
  test('does not throw when underlying write fails', async () => {
    const c = new BleAiClient(makeBleManager({ onWrite: () => Promise.reject(new Error('boom')) }), 'periph');
