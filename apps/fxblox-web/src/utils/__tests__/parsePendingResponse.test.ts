...
describe('parsePendingResponse', () => {
  ...
  test('returns null when trigger is not isolation_mode', () => {
    expect(parsePendingResponse({ ts: '2026-05-24T07:00:00Z', trigger: 'manual', actions: [makeAction()] })).toBeNull();
  });

  test('returns null when ts is missing', () => {
    expect(parsePendingResponse({ trigger: 'isolation_mode', actions: [makeAction()] })).toBeNull();
  });

  test('returns null when actions is not an array', () => {
    expect(parsePendingResponse({ ts: '2026-05-24T07:00:00Z', trigger: 'isolation_mode', actions: 'not-an-array' })).toBeNull();
  });

  test('returns null when all actions are malformed', () => {
    expect(parsePendingResponse({ ts: '2026-05-24T07:00:00Z', trigger: 'isolation_mode', actions: [{ action_id: 'incomplete' }, { type: 'recommended_action' }] })).toBeNull();
  });

  test('parses a minimal valid response (no verdict)', () => {
    const r = parsePendingResponse({ ts: '2026-05-24T07:00:00Z', trigger: 'isolation_mode', actions: [makeAction('a1'), makeAction('a2')] });
  ...
  test('preserves a valid verdict', () => {
    const verdict = { type: 'verdict', payload: { summary: 'wifi flaky', severity: 'yellow', root_cause: 'wifi' } };
    const r = parsePendingResponse({ ts: '2026-05-24T07:00:00Z', trigger: 'isolation_mode', verdict, actions: [makeAction()] });
  ...
  test('skips malformed actions but keeps well-formed ones', () => {
    const r = parsePendingResponse({
      ts: '2026-05-24T07:00:00Z',
      trigger: 'isolation_mode',
      actions: [makeAction('good1'), { type: 'recommended_action', action_id: 'bad' }, makeAction('good2'), { nonsense: true }, makeAction('good3', 3)],
    });
  ...
  test('rejects action with tier 1 (read-only) or invalid tier', () => {
    const r = parsePendingResponse({
      ts: '2026-05-24T07:00:00Z',
      trigger: 'isolation_mode',
      actions: [{ ...makeAction(), tier: 1 }, { ...makeAction(), tier: 4 }, { ...makeAction(), tier: 'two' }, makeAction('keep')],
    });
    expect(r!.actions).toHaveLength(1);
