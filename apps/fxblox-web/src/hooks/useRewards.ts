...
// Ported VERBATIM (import paths) from apps/box/src/hooks/useRewards.ts
import { useState, useEffect, useCallback } from 'react';
import { useRewardOperations } from './useContractIntegration';
import type { RewardInfo } from '@/contracts/types';
...
export const useRewards = (account?: string) => {
  ...
  const loadTotalRewards = useCallback(async () => {
    ...
    try {
      ...
      setState((prev) => ({ ...prev, totalRewards: total, loading: false, error: null }));
    } catch (error: any) {
      console.error('Error loading total rewards:', error);
      setState((prev) => ({ ...prev, totalRewards: null, loading: false, error: error.message || 'Failed to load rewards' }));
    }
  }, [isReady, contractService, targetAccount, rewardOperations]);

  const loadClaimableRewards = useCallback(
    async (poolId: string) => {
      if (!isReady || !contractService || !targetAccount) return;
      try {
        const claimable = await rewardOperations.getClaimableRewards(targetAccount, poolId);
        setState((prev) => ({ ...prev, claimableRewards: claimable }));
        return claimable;
      } catch (error: any) {
  ...
  const loadPoolRewards = useCallback(
    async (poolId: string) => {
      if (!isReady || !contractService || !targetAccount) return;
      try {
        const poolReward = await contractService.getRewards(targetAccount, poolId);
        setState((prev) => ({ ...prev, poolRewards: { ...prev.poolRewards, [poolId]: poolReward } }));
        return poolReward;
      } catch (error: any) {
  ...
  const claimRewards = useCallback(
    async (poolId: string) => {
      ...
      const result = await executeContractCall(() => contractService.claimRewards(poolId), 'Claim Rewards');
      if (result !== null) {
...
export const useMultiPoolRewards = (poolIds: string[], account?: string) => {
    ...
    try {
      const rewardsPromises = poolIds.map(async (poolId) => {
        try {
          ...
          console.error(`Error loading rewards for pool ${poolId}:`, e);
      ...
      results.forEach(({ poolId, reward }) => {
        if (reward) rewardsMap[poolId] = reward;
      });
  ...
  const totalRewards = Object.values(poolRewards).reduce((total, reward) => total + parseFloat(reward.amount), 0);
