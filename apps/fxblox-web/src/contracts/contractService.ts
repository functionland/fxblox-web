...
// Ported VERBATIM from apps/box/src/contracts/contractService.ts (plan WS3 module map). Changes:
//  - `new ethers.providers.Web3Provider(web3Provider, 'any')` so an in-place wallet network switch (browser AppKit)
//    does not throw "underlying network changed" on the cached provider (the plan's one-line change).
//  - strict-TS only: `receipt.events` via ContractReceipt, `provider.request!(...)` (ethers' ExternalProvider
//    marks `request` optional), `(error as Error).message` in one catch. No behavioural change.
...
export class ContractService {
  ...
  async createPool(name: string, region: string, parent: string): Promise<string> {
    try {
      ...
      const event = (receipt as ethers.ContractReceipt).events?.find((e: any) => e.event === 'PoolCreated');
    ...
    try {
      ...
      while (true) {
        try {
          ...
          console.log('📞 Error at index', index, '- assuming end of pools:', (error as Error).message);
          break;
        }
      }
