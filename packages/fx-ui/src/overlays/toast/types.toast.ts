/* Port of libs/component-library/src/lib/toast/types.toast.ts — verbatim except `ColorValue` (RN) → string
   and `JSX.Element` → React.ReactElement. */
...
type OptionalSettings = Pick<ToastProps, 'title' | 'message' | 'onShow' | 'onQueue' | 'onPress'>;
...
export type Color = string; // This covers the color types for both SVG and regular components.
